/*
 * Cellular Automaton - Traffic Flow Simulation (MPI Blocking Version)
 * 
 * Model: 1D circular road with periodic boundary conditions
 * Parallelization: Domain decomposition with blocking halo exchange
 * Communication: MPI_Sendrecv (blocking, deadlock-free)
 * 
 * Compilation:
 *   mpicc -O2 -o ca_mpi_blocking ca_mpi_blocking.c -lm
 * 
 * Usage:
...
void exchange_ghosts_blocking(int *local_old, int local_N, int *ghost_left, int *ghost_right,
                               int left_rank, int right_rank);
int update_local_timestep(int *local_old, int *local_new, int local_N, 
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        printf("Cellular Automaton Traffic Simulation (MPI Blocking)\n");
        printf("========================================================\n");
        printf("Parameters:\n");
    ...
    for (int t = 0; t < T; t++) {
        // Step 1: Exchange ghost cells (blocking communication)
        exchange_ghosts_blocking(local_old, local_N, &ghost_left, &ghost_right,
                                 left_rank, right_rank);
        
        // Step 2: Update local cells using CA rules
        int local_moved = update_local_timestep(local_old, local_new, local_N,
                                                ghost_left, ghost_right);
        
        // Step 3: Global reduction to sum moved cars
        int global_moved = 0;
        MPI_Reduce(&local_moved, &global_moved, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
        
        // Step 4: Calculate and store velocity (rank 0 only)
        if (rank == 0) {
        ...
        // Step 5: Swap buffers (old becomes new)
        int *temp = local_old;
        local_old = local_new;
        local_new = temp;
    }
...
/**
 * Exchange ghost cells with neighbors using blocking communication
 * Uses MPI_Sendrecv for deadlock-free exchange
 */
void exchange_ghosts_blocking(int *local_old, int local_N, int *ghost_left, int *ghost_right,
                               int left_rank, int right_rank) {
    int rank, size;
    ...
    // Non-blocking receives first (to avoid deadlock)
    MPI_Irecv(ghost_left, 1, MPI_INT, left_rank, 0, MPI_COMM_WORLD, &requests[0]);
    MPI_Irecv(ghost_right, 1, MPI_INT, right_rank, 1, MPI_COMM_WORLD, &requests[1]);
    
    // Then blocking sends
    MPI_Send(&local_old[local_N - 1], 1, MPI_INT, right_rank, 0, MPI_COMM_WORLD);
    MPI_Send(&local_old[0], 1, MPI_INT, left_rank, 1, MPI_COMM_WORLD);
...
/**
 * Update local cells for one timestep using CA rules
 * Returns number of cars that moved in this local domain
 */
int update_local_timestep(int *local_old, int *local_new, int local_N,
                          int ghost_left, int ghost_right) {
    int moved_count = 0;
    ...
    for (int i = 0; i < local_N; i++) {
        // Get neighbors (handle boundaries with ghost cells)
        int left, curr, right;
    ...
    // Count how many cars moved (compare old vs new)
    for (int i = 0; i < local_N; i++) {
